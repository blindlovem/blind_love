package com.example.loveapp.util.bazi.fuben;
...
public class LuozhuangshenshaHehun {
     ...
     */
    public String ceyixia(String s) throws ParseException {

        Calendar mancal;
        Calendar womancal;
        try {
            mancal = getCalendarfromString(s, "yyyy-MM-dd HH");
           // womancal = getCalendarfromString(woman, "yyyy-MM-dd HH");
        } catch (ParseException ex) {
            return "输入不正确" + ex.getMessage();
        }

        return shensha(mancal);
    ...
    public String shensha(Calendar cal) throws ParseException {
        ...
        System.out.println("此人八字【" + lunar.getYearGanZhi(time) + "】"); //获取生肖 System.out.println("此人的农历生肖【" + lunar.animalsYear() + "】"); String GanZhi = lunar.getYearGanZhi(time);//取八字 ;//取表格中的中国农历月份 //取月份，其实原来表格不很合理，因为没有考虑闰月和节气问题 //正规算八字按节气算，即使闰月也不怕——大师♂罗莊
        //建议按照文章中根据五行纳音或者属相来处理某些神煞 //这里要帮大家解决通用问题，我就把四柱和四柱五行纳音都取出来 //然后大家根据substring 取出五行和属相 String[] tempchar = GanZhi.split(","); //我修改原来的，用,分割 String ganziyear = tempchar[0];//年柱 String ganzimonth = tempchar[1];//月柱 String ganziday = tempchar[2];//日柱 String ganzitime
 //= tempchar[3];//时柱 //五行纳音 String soundyear = getnumsix(ganziyear); String soundmonth = getnumsix(ganzimonth); String soundday = getnumsix(ganziday); String soundtime = getnumsix(ganzitime); //表格有14格 //其中属相从数组1-12，顺序就是子1，丑2这样 //使用者注意：本代码只是简单取表，没有考虑男女和多月分以及五行纳音
        int number = lunar.getYearindex() + 1; StringBuffer result = new StringBuffer(); for (int i = 0; i < 14; i++) { if (month.equalsIgnoreCase(shensha1[number][ i])) { result.append("此人犯" + shensha1[0][ i]); result.append("\n"); } if (month.equalsIgnoreCase(shensha2[number][
                i])) { result.append("此人犯" + shensha2[0][ i]); result.append("\n"); } } return result.toString(); } /** * 字符串-》数字 * * @param year 字符串 * @return 字符串对应数字 * @throws NumberFormatException */ public int getnumber(String year) throws NumberFormatException { int
            yearnumber = Short.parseShort(year); return yearnumber; } /** * * * @return 传回农历 y年的属相 */ final String baiziYear(int year) { return Zhi[(year - 4) % 12]; } public static void main(String[] args) { LuozhuangshenshaHehun my = new LuozhuangshenshaHehun(); try
    { //System.out.println(my.hehun("2001-2-20 1", "2004-9-6 4"));
        System.out.println(my.ceyixia("2023-05-23 5"));

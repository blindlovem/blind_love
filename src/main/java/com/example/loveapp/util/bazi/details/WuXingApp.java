package com.example.loveapp.util.bazi.details;

/*
* 先根据五行相生相克匹配：
*   1.传入user，计算得到八字五行，以及五行旺 与 五行缺
*   2.传入已经基本匹配得到的用户列表，遍历得到每个用户的五行情况并记录
*   3.边遍历，边比较列表用户与user的五行关系，计算得分，最终选择出4个
*
*
* */

import com.example.loveapp.entity.User;
//import com.example.loveapp.util.nature.UserScore;
...
public class WuXingApp {
    ...
    /**
     * 匹配寻找基于五行最合适的四个用户，以列表形式返回
     * @param targetUser
     * @param allUsers
     * @return
     */
    public static List<User> findMatchesByBaZi(User targetUser, List<User> allUsers){

        List<UserScore> userScores = new ArrayList<>();

        for (User user : allUsers) {
            double overallScore = wxScore(targetUser,user);

            UserScore userScore = new UserScore(user, overallScore);
            userScores.add(userScore);
        }

        // 根据得分从高到低排序，并选出最合适的两个用户
        userScores.sort((o1, o2) -> Double.compare(o2.getScore(), o1.getScore()));
        List<User> bestMatches = new ArrayList<>();
        if (userScores.size() >= 2) { // TODO 以后改成四
            bestMatches.add(userScores.get(0).getUser());
            bestMatches.add(userScores.get(1).getUser());
//            bestMatches.add(userScores.get(2).getUser());
//            bestMatches.add(userScores.get(3).getUser());
        }

        return bestMatches;


        //return allUsers; //TODO 需要编写算法
    }
...
//    }
     ...
     */
    public static String wuxing(String gzYear, String gzMonth, String gzDay, String gzTime) {
        // 天干年
...
五行相克：金克木、木克土、土克水、水克火、火克金。*/
    private static HashMap<String,String> sheng = new HashMap<>();

    private  static HashMap<String,String> ke = new HashMap<>();
    ...
    /**
     * u1五行中最旺盛的如果可以生出u2中缺的  加4分   相互 则加8分
     * u1五行中最旺盛的如果是u2中缺的      加2分   相互 则加4分
     *
     * 最旺的相互克   加-3分
     *
     * u1五行中有可以生u2缺的    加2分 ；  有克的  加-1分
     *
     * 其余为 0分
     * 用户八字只填写：哪个小时出生的：格式：8/18
     * 计算两人五行得分，
     * @param u1
     * @param u2
     * @return
     */
    public static double wxScore(User u1, User u2){

        LocalDate date1 = u1.getBirthday().toLocalDate();
        int year1 = date1.getYear();
        int month1 = date1.getMonthValue();
        int day1 = date1.getDayOfMonth();
        int time1 = Integer.parseInt(u1.getBazi());

        LocalDate date2 = u2.getBirthday().toLocalDate();
        int year2 = date2.getYear();
        int month2 = date2.getMonthValue();
        int day2 = date2.getDayOfMonth();
        int time2 = Integer.parseInt(u2.getBazi());


        // 获取干支年
        String gzYear1 = getYear(year1);


        // 获取干支月
        String gzMonth1 = getMonth(gzYear1, month1);

        // 获取干支日
        String gzDay1 = getDay(year1, month1, day1);


        // 获取干支时
        String gzTime1 = getTime(gzDay1, time1);


        String wuxing1 = wuxing(gzYear1, gzMonth1, gzDay1, gzTime1).split("&")[0];
        int[] cnt = {0,0,0,0,0};
        for (int i = 0;i < wuxing1.length();i++){
            if (wuxing1.charAt(i) == '金'){
                cnt[0]+=1;
            }
            if (wuxing1.charAt(i) == '木'){
                cnt[1]+=1;
            }
            if (wuxing1.charAt(i) == '水'){
                cnt[2]+=1;
            }
            if (wuxing1.charAt(i) == '火'){
                cnt[3]+=1;
            }
            if (wuxing1.charAt(i) == '土'){
                cnt[4]+=1;
            }
        ...
        String wang1 = wuxing1.charAt(index) + "";
        String que1 = wuxing(gzYear1, gzMonth1, gzDay1, gzTime1).split("&")[1];

        // 获取干支年
        String gzYear2 = getYear(year2);
        //System.out.println(year + " 年为 \"" + gzYear + "\" 年");

        // 获取干支月
        String gzMonth2 = getMonth(gzYear2, month2);


        // 获取干支日
        String gzDay2 = getDay(year2, month2, day2);


        // 获取干支时
        String gzTime2 = getTime(gzDay2, time2);


        String wuxing2 = wuxing(gzYear2, gzMonth2, gzDay2, gzTime2).split("&")[0];
        int[] cnt1 = {0,0,0,0,0};
        for (int i = 0;i < wuxing1.length();i++){
            if (wuxing2.charAt(i) == '金'){
                cnt1[0]+=1;
            }
            if (wuxing2.charAt(i) == '木'){
                cnt1[1]+=1;
            }
            if (wuxing2.charAt(i) == '水'){
                cnt1[2]+=1;
            }
            if (wuxing2.charAt(i) == '火'){
                cnt1[3]+=1;
            }
            if (wuxing2.charAt(i) == '土'){
                cnt1[4]+=1;
            }
        ...
        String wang2 = wuxing2.charAt(index1) + "";
        String que2 = wuxing(gzYear2, gzMonth2, gzDay2, gzTime2).split("&")[1];



        double score = 0;

        //u1五行中最旺盛的如果可以生出u2中缺的
        if (sheng.get(wang1).equals(que2)){
            score += 4;
        }
        if (sheng.get(wang2).equals(que1)){
            score += 4;
        }
        //u1五行中最旺盛的如果是u2中缺的
        if (wang1.equals(que2)){
            score+=2;
        }
        if (wang2.equals(que1)){
            score+=2;
        }
        //最旺的相互克
        if (ke.get(wang1).equals(wang2)){
            score-=3;
        }
        if (ke.get(wang2).equals(wang1)){
            score-=3;
        }
        //u1五行中有可以生u2缺的    加2分
        for (int i = 0;i<wuxing1.length();i++){
            if (sheng.get(wuxing1.charAt(i) + "").equals(que2)){
                score+=2;
            }
        }
        for (int i = 0;i<wuxing2.length();i++){
            if (sheng.get(wuxing2.charAt(i) + "").equals(que1)){
                score+=2;
            }
        }
        ...
        //有克的  加-1分
        for (int i = 0;i<wuxing1.length();i++){
            for (int j = 0;j<wuxing2.length();j++){
                if (ke.get(wuxing1.charAt(i) + "").equals(wuxing2.charAt(j) + "")){
                    score-=1;
                }
            }
        }
        ...
        for (int i = 0;i<wuxing2.length();i++){
            for (int j = 0;j<wuxing1.length();j++){
                if (ke.get(wuxing2.charAt(i) + "").equals(wuxing1.charAt(j) + "")){
                    score-=1;
                }
            }
        }
